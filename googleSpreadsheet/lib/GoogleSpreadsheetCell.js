...
class GoogleSpreadsheetCell {
  ...
  get value() {
    ...
    if (this._draftData.value !== undefined)
  ...
  set value(newValue) {
    if (_.isBoolean(newValue)) {
      ...
      if (newValue.substr(0, 1) === '=')
        this._draftData.valueType = 'formulaValue'
  ...
  set hyperlink(val) {
    throw new Error(
      'Do not set hyperlink directly. Instead set cell.formula, for example `cell.formula = \'=HYPERLINK("http://google.com", "Google")\'`'
    )
  }

  get note() {
    return this._draftData.note !== undefined
      ? this._draftData.note
      : this._rawData.note
  }
  ...
  _getFormatParam(param) {
    ...
    if (_.get(this._draftData, `userEnteredFormat.${param}`)) {
      throw new Error(
        'User format is unsaved - save the cell to be able to read it again'
      )
    }
  ...
  _getUpdateRequest() {
    ...
    const isFormatUpdated = !!_.keys(this._draftData.userEnteredFormat || {})
      .length
    ...
    if (
      !_.some([isValueUpdated, isNoteUpdated, isFormatUpdated, isFormatCleared])
    ) {
      return null
    }
